...
class Submissions extends Component {
	...
	renderHelperSubmissionForm() {
		if (this.isSubmissionEnabled()) {
			...
			let button = (
				<button disabled style={{ float: "right" }} onClick={this.uploadData} className={btn_class}>
					{" "}
					Submit{" "}
				</button>
			);
			if (this.state.selectedFile !== null) {
				...
				button = (
					<div style={{ float: "right" }}>
						...
						<button onClick={this.uploadData} className={btn_class}>
							{" "}
							Submit{" "}
						</button>
					</div>
			...
			return (
				<div className="card">
					...
					<div className="content">
						<p>
							Create a <code>zip</code> file of your robot player, and submit it below. The submission format should be
							a zip file that has the <b>same name</b> of your package (e.g.: <code>examplefuncsplayer.zip</code>),
							containing your package folder (e.g.: <code>examplefuncsplayer</code>), that should contain a{" "}
							<code>RobotPlayer.java</code> and any other code you have written. Your team should give the package a{" "}
							<b>unique name</b>. If the package name is taken by other team, you'll see an error message beside the
							submit button.
							<br /> (After changing the package name from <code>examplefuncsplayer</code> to something else, remember
							to update the <b>first line</b> of every <code>.java</code> file, from{" "}
							<code>package examplefuncsplayer;</code> to <code>package [youruniquepackagename];</code>)
						</p>
						<pre>
							<code>examplefuncsplayer.zip --&gt; examplefuncsplayer ---&gt; RobotPlayer.java, xxx.java</code>
						</pre>
						<p>If you're having trouble submitting:</p>
						<ul>
							<li>
								Try to compile it locally before submitting. You can run this gradle task{" "}
								<code>./gradlew run -PteamA=[yourrobot] -PteamB=[yourrobot]</code> in the terminal to check the log and
								see if your code throws any exceptions.
							</li>
							<li>Check that your zip contains exactly one directory, and your code is inside that directory.</li>
							<li>
								Non-ASCII characters: Ensure your code is completely ASCII. In the past we have had compile errors due
								to comments containing diacritic characters (áéíóú).
							</li>
							<li>
								Make sure you only import from your own bot, and from java. packages. In particular, do not use javax,
								javafx, and watch out for importing from other versions of your bot (which may work locally, but will
								not work on our servers as you can only submit one folder).
							</li>
							<li>
								If it's <b>queuing</b>, refresh the page every 3 minutes, because there might be submissions ahead of
								you. If it's <b>queuing</b> for hours, contact Li.
							</li>
							<li>
								If it says <b>compile failed</b>, most of time your code genuinely can't compile. As of Sprint Two the
								submission threshold will increase further.
							</li>
							<li>
								Send the zip to <a href="mailto:cecishi@pdx.edu">cecishi@pdx.edu</a> before deadline. (Please grant me a
								10-minute's grace.)
							</li>
						</ul>
						<label htmlFor="file_upload">
							<div className="btn"> Choose File </div>{" "}
							<span style={{ textTransform: "none", marginLeft: "10px", fontSize: "14px" }}> {file_label} </span>
						</label>
						<input
							id="file_upload"
							type="file"
	...
	renderHelperLastTable() {
		if (this.state.lastSubmissions === null) {
			...
			if (this.state.status === 0) {
				return <p>Your code is currently compiling—you'll see it here if it finishes successfully.</p>;
			} else {
							...
							{submission.status === "compiled" ? (
								<td>
									...
									<button
										...
										onClick={() => this.onSubFileRequest(submission.name, submission.numDate)}
									>
										Download
									</button>{" "}
								</td>
