...
class Api {
    ...
    if (teamName && teamKey) {
      ...
      if (robotSnap.exists() && robotSnap.data().submissions) {
        ...
        allSubmission.find((o, i) => {
          if (
            o.robot ===
              submissionFile.name.slice(0, submissionFile.name.length - 4) &&
            o.teamname !== teamName
          ) {
        ...
        if (!uploadFailed) {
          ...
          if (teamSnap.exists()) {
            ...
            const storageRef = ref(
              storage,
              teamName + "/" + dateNumVersion + submissionFile.name
            );
  ...
  static downloadSubmission(submissionName, numDate, callback) {
    if (Cookies.get("teamName")) {
      getDownloadURL(
        ref(storage, Cookies.get("teamName") + "/" + numDate + submissionName)
      )
        .then((url) => {
          console.log(url);
          callback(true);
    ...
    if (docSnap.exists() && docSnap.data().scrimmages) {
      ...
      scrimmagesData.find((o, i) => {
        if (o.Elo === false) {
          ...
          var won =
            o.STATUS === "Awon" ? o.TEAMA : o.STATUS === "Bwon" ? o.TEAMB : "";
          var los =
            o.STATUS === "Awon" ? o.TEAMB : o.STATUS === "Bwon" ? o.TEAMA : "";
          var wonIndex;
          var losIndex;
              ...
              (1 +
                1.0 *
                  Math.pow(
                    10,
                    (1.0 *
                      (allTeam[losIndex].score - allTeam[wonIndex].score)) /
                      400
                  ));
            var probL = 1 - probW;
  ...
  static downloadScrimmage(date, robotOne, robotTwo, map) {
    getDownloadURL(
      ref(storage, date + robotOne + "-vs-" + robotTwo + "-on-" + map + ".zip")
    )
      .then((url) => {
        console.log(url);
        const aHelper = document.createElement("a");
        ...
        .then(() => {
          try {
            ...
            updateProfile(auth.currentUser, {
              ...
              .catch((error) => {
                console.log(
                  "firebase update user display name error: " + error
                );
              });
          } catch (error) {
