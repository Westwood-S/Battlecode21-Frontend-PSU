    ...
    this.getAllTeamScrimmages((scrimmages) => {
      const requests = scrimmages
        .filter((scrimmage) => {
          ...
          return (
            scrimmage.requested_by !== parseInt(Cookies.get('team_id'), 10)
          );
        })
        .map((scrimmage) => {
  ...
  */
  ...
  static getScrimmageHistory(callback, page) {
    ...
    this.getTeamScrimmages((s, count) => {
      ...
      for (let i = 0; i < s.length; i++) {
        ...
        s[i].status =
          s[i].status.charAt(0).toUpperCase() + s[i].status.slice(1);
  ...
  static getNextTournament(callback) {
    ...
    callback({
      ...
      seconds_until:
        (Date.parse(new Date('January 28, 2021 19:00:00-5:00')) -
          Date.parse(new Date())) /
        1000,
